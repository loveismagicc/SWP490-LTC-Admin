...
const BookingDetail = () => {
    ...
    useEffect(() => {
        if (roomId) {
            ...
            roomService
                ...
                .then((data) => {
					let room = {...data};
					if(room?.hotelId && typeof room.hotelId === "object" && room.hotelId._id) {
						room.hotel_id = room.hotelId._id;
					} else if(typeof room.hotelId === "string" && !room.hotel_id) {
						room.hotel_id = room.hotelId;
					}
					room.amenities = Array.isArray(room.amenities) ? room.amenities.join(", ") : room.amenities;
                    setForm(room);
                    if (room?.availability?.length > 0) {
                        setQuantity(room.availability[0].quantity);
                    }
                })
                .catch(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = {
				...
				amenities: form.amenities.split(",").map((v) => v.trim()),
                availability: [
                    {
                        date: new Date(),
    ...
    const isEditable = true // || user?.role === "hotel_owner";
